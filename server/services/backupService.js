...
// Writable backups dir (per-user app-data in the packaged desktop app; survives
// uninstall/reinstall). Falls back to the server folder in dev. See config/paths.js.
...
//
// Every model listed here MUST carry a `business` field, because restore scopes by
// { business } (getBaseFilter) and re-stamps { user, business } (getCreateData).
//
// DELIBERATELY EXCLUDED (do NOT add): identity/access models (User, Business, Branch,
// Role) — restore runs INSIDE an existing business/user context and re-stamps ownership,
// so wiping/duplicating these would corrupt logins or lock out staff mid-session;
// AuditLog — an immutable trail whose actor must not be rewritten to the restoring user;
// DriveBackup — holds Google OAuth refresh tokens (secrets, device/cloud-bound);
// Setting — keyed by `user` not `business`, restored via its own path; and ephemeral/
// system collections (Otp, PushSubscription, Notification, *History, Barcode*, WhatsAppSession).
const TENANT_MODELS = {
  // core ledger & parties
  sales: 'Sale',
  purchases: 'Purchase',
  ...
  accounts: 'Account',
  // inventory (stockMovements is critical — stock balances are derived from it)
  ...
  partyTransfers: 'PartyToPartyTransfer',
  ledgerNotes: 'LedgerNote',
  loyaltyPoints: 'LoyaltyPoint',
  // config & misc tenant data
  budgets: 'Budget',
  currencies: 'Currency',
  ...
  support: 'Support',
  // numbering sequences — restored so invoice/bill numbers stay continuous
  counters: 'Counter',
};
