...
const columns = [
  { key: 'gstRate', label: 'GST Rate', width: 'w-[120px]' },
  { key: 'taxableAmount', label: 'Taxable Amount', width: 'w-[150px]', align: 'right', render: (v) => `₹${(v||0).toLocaleString()}` },
  { key: 'cgst', label: 'CGST', width: 'w-[130px]', align: 'right', render: (v) => `₹${(v||0).toLocaleString()}` },
  { key: 'sgst', label: 'SGST', width: 'w-[130px]', align: 'right', render: (v) => `₹${(v||0).toLocaleString()}` },
  { key: 'igst', label: 'IGST', width: 'w-[130px]', align: 'right', render: (v) => `₹${(v||0).toLocaleString()}` },
  { key: 'totalTax', label: 'Total Tax', width: 'w-[130px]', align: 'right', render: (v) => `₹${(v||0).toLocaleString()}` },
];

const rateRows = ['0%', '5%', '12%', '18%', '28%'];
...
const GSTRateReport = () => {
  ...
  const mappedData = rateRows.map(rate => {
    ...
    return {
      ...
      sgst: entry.sgst || 0,
      igst: entry.igst || 0,
      totalTax: (entry.cgst || 0) + (entry.sgst || 0) + (entry.igst || 0),
    };
  });
  const filteredMappedData = mappedData.filter(d => !search || Object.values(d).some(v => String(v).toLowerCase().includes(search.toLowerCase())));

  const totalRow = mappedData.reduce((acc, row) => ({
            ...
            <table className="w-full text-xs whitespace-nowrap">
              ...
              <tfoot>
                <tr className="bg-gray-50 dark:bg-[#111827] border-t border-gray-200 dark:border-[#334155]">
                  <td className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0]">Grand Total</td>
                  <td className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">₹{totalRow.taxableAmount.toLocaleString()}</td>
                  <td className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">₹{totalRow.cgst.toLocaleString()}</td>
                  <td className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">₹{totalRow.sgst.toLocaleString()}</td>
                  <td className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">₹{totalRow.igst.toLocaleString()}</td>
                  <td className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">₹{totalRow.totalTax.toLocaleString()}</td>
                </tr>
              </tfoot>
            </table>
          </div>
