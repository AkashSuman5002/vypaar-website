...
const columns = [
  ...
  { key: 'invoiceNo', label: 'Invoice No', width: 'w-[130px]' },
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await reportAPI.getTDSReceivable(params);
        setData(res.data.entries || []);
      } catch (err) { console.error('Failed to load TDS Receivable', err); }
    ...
    <div className="bg-white dark:bg-[#0F172A] min-h-full flex flex-col">
      <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-200 dark:border-[#334155]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="px-3 py-1.5 text-xs font-medium rounded-md bg-white dark:bg-[#1E293B] text-gray-600 dark:text-[#94A3B8] border border-gray-300 dark:border-[#334155] hover:bg-gray-50 dark:hover:bg-[#1E293B]/70" onClick={() => exportToExcel(filteredData, columns, 'TDS_Receivable')}>Excel</button>
            ...
            <table className="w-full text-xs whitespace-nowrap">
              ...
              <tfoot>
                <tr className="bg-gray-50 dark:bg-[#111827] border-t border-gray-200 dark:border-[#334155]">
                  <td colSpan={6} className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">Total Receivable</td>
                  <td className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">₹{totalReceivable.toLocaleString()}</td>
                </tr>
              </tfoot>
            </table>
          </div>
