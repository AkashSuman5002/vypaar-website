...
const columns = [
  ...
  { key: 'vendorName', label: 'Vendor Name', width: 'w-[170px]' },
  { key: 'billNo', label: 'Bill No', width: 'w-[130px]' },
  { key: 'taxableAmount', label: 'Taxable Amount', width: 'w-[140px]', align: 'right', render: (v) => `₹${(v||0).toLocaleString()}` },
  { key: 'tdsPct', label: 'TDS %', width: 'w-[90px]', align: 'right', render: (v) => `${(v||0)}%` },
  { key: 'tdsAmount', label: 'TDS Amount', width: 'w-[130px]', align: 'right', render: (v) => `₹${(v||0).toLocaleString()}` },
  { key: 'payableAmount', label: 'Payable Amount', width: 'w-[150px]', align: 'right', render: (v) => `₹${(v||0).toLocaleString()}` },
];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await reportAPI.getTDSPayable(params);
        setData(res.data.entries || []);
      } catch (err) { console.error('Failed to load TDS Payable', err); }
      finally { setLoading(false); }
    };
    fetchData();
  }, [dateFrom, dateTo]);

  const totalPayable = data.reduce((s, d) => s + (d.payableAmount || 0), 0);
    ...
    <div className="bg-white dark:bg-[#0F172A] min-h-full flex flex-col">
      <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-200 dark:border-[#334155]">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="px-3 py-1.5 text-xs font-medium rounded-md bg-white dark:bg-[#1E293B] text-gray-600 dark:text-[#94A3B8] border border-gray-300 dark:border-[#334155] hover:bg-gray-50 dark:hover:bg-[#1E293B]/70" onClick={() => exportToExcel(filteredData, columns, 'TDS_Payable')}>Excel</button>
            ...
            <table className="w-full text-xs whitespace-nowrap">
              ...
              <tfoot>
                <tr className="bg-gray-50 dark:bg-[#111827] border-t border-gray-200 dark:border-[#334155]">
                  <td colSpan={6} className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">Total Payable</td>
                  <td className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">₹{totalPayable.toLocaleString()}</td>
                </tr>
              </tfoot>
            </table>
          </div>
