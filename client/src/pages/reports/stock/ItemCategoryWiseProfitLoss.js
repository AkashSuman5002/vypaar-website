...
const columns = [
  ...
  { key: 'category', label: 'Category Name', width: 'w-[200px]' },
...
const ItemCategoryWiseProfitLoss = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await reportAPI.getItemCategoryProfitLoss(params);
        setData(res.data.entries || []);
        setTotals(res.data);
      } catch (err) {
        console.error('Failed to load item category profit loss', err);
        setData([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="bg-white dark:bg-[#0F172A] min-h-full flex flex-col">
      <StockHeader title="Item Category Wise Profit And Loss" search={search} onSearchChange={setSearch} />
      ...
      <div className="flex-1 px-4 pb-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex gap-2 ml-auto">
            <button onClick={() => exportToExcel(data, columns, 'Item_Category_Profit_Loss')} className="px-3 py-1.5 text-xs font-medium rounded-md bg-white dark:bg-[#1E293B] text-gray-600 dark:text-[#94A3B8] border border-gray-300 dark:border-[#334155] hover:bg-gray-50 dark:hover:bg-[#1E293B]/70">Excel</button>
            ...
            <table className="w-full text-xs whitespace-nowrap">
              ...
              <tbody>
                {data.length === 0 ? (
                  ...
                  <tr key={idx} className="border-b border-gray-100 dark:border-[#334155]/50 hover:bg-gray-50 dark:hover:bg-[#1E293B]/40">
                    ...
                    <td className="px-3 py-2.5 text-gray-900 dark:text-[#F8FAFC]">{item.category}</td>
              ...
              <tfoot>
                <tr className="bg-gray-50 dark:bg-[#111827] border-t border-gray-200 dark:border-[#334155]">
                  <td colSpan={2} className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">Category Totals</td>
