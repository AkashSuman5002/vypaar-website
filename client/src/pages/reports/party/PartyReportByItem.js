...
const columns = [
  ...
  { key: 'party', label: 'Party Name', width: 'w-[180px]' },
  { key: 'item', label: 'Item', width: 'w-[150px]' },
  { key: 'saleQty', label: 'Sale Quantity', width: 'w-[110px]', align: 'right' },
  { key: 'saleAmount', label: 'Sale Amount', width: 'w-[130px]', align: 'right' },
  { key: 'purchaseQty', label: 'Purchase Quantity', width: 'w-[120px]', align: 'right' },
  { key: 'purchaseAmount', label: 'Purchase Amount', width: 'w-[130px]', align: 'right' },
];
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await reportAPI.getPartyReportByItem(params);
        setData(res.data.entries || []);
      } catch (err) {
        console.error('Failed to load party report by item', err);
        setData([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="p-4 bg-white dark:bg-[#0F172A] min-h-full">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-gray-900 dark:text-[#F8FAFC]">Party Report By Item</h2>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => exportToExcel(data, columns, 'Party_Report_By_Item')} className="px-3 py-1.5 text-xs font-medium rounded-md bg-white dark:bg-[#1E293B] text-gray-600 dark:text-[#94A3B8] border border-gray-300 dark:border-[#334155] hover:bg-gray-50 dark:hover:bg-[#1E293B]/70">Excel</button>
        ...
        <table className="w-full text-xs">
            ...
            {data.length === 0 ? (
              <tr>
                <td colSpan={columns.length} className="px-3 py-12 text-center text-sm text-gray-500 dark:text-[#64748B]">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <span className="text-sm text-gray-500 dark:text-[#64748B]">No data is available for Party Report by Item</span>
                  </div>
                </td>
              </tr>
            ) : (
              data.map((item, idx) => (
                <tr key={idx} className="border-b border-gray-200 dark:border-[#334155]">
                  ...
                  <td className="px-3 py-2.5 text-gray-900 dark:text-[#F8FAFC]">{item.party}</td>
                  <td className="px-3 py-2.5 text-gray-500 dark:text-[#64748B]">{item.item}</td>
                  <td className="px-3 py-2.5 text-gray-900 dark:text-[#F8FAFC] text-right">{item.saleQty}</td>
                  <td className="px-3 py-2.5 text-gray-900 dark:text-[#F8FAFC] text-right">₹{(item.saleAmount ?? 0).toLocaleString()}</td>
                  <td className="px-3 py-2.5 text-gray-900 dark:text-[#F8FAFC] text-right">{item.purchaseQty}</td>
          ...
          <tfoot>
            <tr className="bg-gray-50 dark:bg-[#111827] border-t border-gray-200 dark:border-[#334155]">
              ...
              <td className="px-3 py-2.5 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">{data.reduce((s, d) => s + (d.saleQty || 0), 0)}</td>
              ...
              <td className="px-3 py-2.5 text-xs font-semibold text-gray-700 dark:text-[#E2E8F0] text-right">{data.reduce((s, d) => s + (d.purchaseQty || 0), 0)}</td>
