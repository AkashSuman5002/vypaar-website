...
const apiMap = {
  ...
  'Sale/Purchase Report By Item Category': (params) => reportAPI.getSalePurchaseByItemCategory(params).then(r => (r.data.entries||[]).map(e => ({category:e.category,sales:e.sales||0,purchases:e.purchases||0}))),
  'Stock Summary Report By Item Category': (params) => reportAPI.getStockSummaryByItemCategory(params).then(r => (r.data.entries||[]).map(e => ({category:e.category,items:e.items||0,qty:e.qty||0,value:e.value||0}))),
  'Item Wise Discount': (params) => reportAPI.getItemWiseDiscount(params).then(r => (r.data.entries||[]).map(e => ({item:e.item,rate:parseFloat(e.discountRate)||0,amount:e.totalDiscount||0}))),
