...
import { Printer, ArrowLeft, Download, Phone, Mail, MapPin, BadgePercent, Landmark, QrCode, Edit3, Copy, MessageSquare, Trash2, FileText, ChevronDown, ShoppingCart } from 'lucide-react';
...
const ViewOrder = () => {
  ...
  if (!sale) return <div className="text-center py-16 text-slate-400 dark:text-slate-500">Sale order not found</div>;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-[210mm] mx-auto">
      <div className="no-print flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        <button onClick={() => navigate('/sales/orders')} className="inline-flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {sale.status !== 'cancelled' && (
            ...
            <ShoppingCart className="w-4 h-4" /> Convert to Invoice
          </button>
          )}
          <Link to={`/sales/orders/${sale._id}/edit`} className="inline-flex items-center gap-1.5 px-3 py-2 border border-slate-200/80 dark:border-gray-700/80 text-blue-600 dark:text-blue-400 text-sm font-medium rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors">
            <Edit3 className="w-4 h-4" /> Edit
          </Link>
          <button onClick={async () => {
            try { await saleAPI.duplicate(sale._id); toast.success('Duplicated'); navigate('/sales/orders'); }
            catch { toast.error('Failed'); }
          ...
          <button onClick={async () => {
            ...
            const caption = `*Sale Order ${sale.invoiceNumber}*\nAmount: ${formatCurrency(sale.totalAmount)}`;
            try {
              await shareDocumentToWhatsApp({ type: 'order', id: sale._id, phone, caption, fileName: `Order-${sale.invoiceNumber}` });
          ...
          <div className="relative">
            ...
            {showActions && (
              <div className="absolute right-0 top-full mt-1 z-50 bg-white dark:bg-gray-800 rounded-xl border border-slate-200 dark:border-gray-700 shadow-lg py-1 min-w-[200px]" onMouseLeave={() => setShowActions(false)}>
                ...
                }} className="flex items-center gap-2 w-full px-3 py-2 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-gray-700"><FileText className="w-3.5 h-3.5" /> Convert to Challan</button>
                <div className="h-px bg-slate-100 dark:bg-gray-700 my-1" />
                <button onClick={() => {
                  if (!window.confirm('Cancel this sale order?')) return;
                  saleAPI.delete(sale._id).then(() => { toast.success('Cancelled'); navigate('/sales/orders'); }).catch(() => toast.error('Failed'));
                }} className="flex items-center gap-2 w-full px-3 py-2 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10"><Trash2 className="w-3.5 h-3.5" /> Cancel Order</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-6 sm:p-8 md:p-10 print:p-6">
          <div className="flex flex-col sm:flex-row justify-between gap-6 pb-6 border-b-2 border-gray-100 dark:border-gray-700 print:border-gray-300 mb-6">
            ...
            <div className="text-left sm:text-right">
              <h2 className="font-bold text-cyan-600 dark:text-cyan-400 print:text-cyan-700 mt-3" style={{ fontSize: `${invoiceHeadingSize + 4}px` }}>SALE ORDER</h2>
              <div className="mt-3 space-y-1">
                <p className="text-sm text-slate-900 dark:text-slate-100 print:text-gray-900"><span className="text-slate-500 dark:text-slate-400 print:text-gray-600">Order: </span><span className="font-semibold">{sale.invoiceNumber}</span></p>
                ...
                <p className="text-sm text-slate-900 dark:text-slate-100 print:text-gray-900"><span className="text-slate-500 dark:text-slate-400 print:text-gray-600">Expected Delivery: </span><span className="font-semibold">{formatDateOrTime(sale.dueDate || sale.date)}</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 print:border-gray-300 pt-6">
            <div className="flex flex-col sm:flex-row justify-between gap-6">
              <div>
                <p className="text-sm text-slate-900 dark:text-slate-100 print:text-gray-900 font-medium">Thank you for your order!</p>
              ...
              <div className="text-left sm:text-right">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 print:text-gray-500 mt-1 max-w-xs sm:ml-auto leading-relaxed">{sale.termsConditions || 'Order is subject to stock availability.'}</p>
                </>
                )}
