    ...
    return (
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
        <motion.div variants={itemVariants}>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Manage returns and debit notes</p>
        </motion.div>
        <motion.div variants={itemVariants}
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
        ...
        >
          <div className="flex flex-col items-center text-center max-w-sm mx-auto">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">No Debit Notes Yet</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Record purchase returns and issue debit notes to suppliers.</p>
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={itemVariants} className="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 shadow-soft p-4">
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            }} className="inline-flex items-center gap-1.5 px-3.5 py-2.5 bg-slate-50 dark:bg-gray-700 border border-slate-200 dark:border-gray-700 rounded-xl text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-gray-700/60 transition-colors">
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 shadow-soft p-4 lg:p-5">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Returns</p><p className="text-xl font-bold text-slate-900 dark:text-slate-100 mt-1">{debits.length}</p></div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 shadow-soft p-4 lg:p-5">
          <div className="flex items-center gap-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 shadow-soft p-4 lg:p-5">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Pending Returns</p><p className="text-xl font-bold text-slate-900 dark:text-slate-100 mt-1">{debits.filter(d => d.reason !== 'Resolved').length}</p></div>
          </div>
        </div>
      </motion.div>
            ...
            <tbody>
              {paginated.map((d, idx) => (
                <tr key={d._id} className="group border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{formatDate(d.returnDate)}</td>
                  <td className="px-4 py-3"><span className="text-sm font-semibold text-slate-900 dark:text-slate-100 font-mono">{d.returnNumber}</span></td>
                  <td className="px-4 py-3"><span className="text-sm font-medium text-slate-900 dark:text-slate-100">{d.supplierName}</span></td>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-sm text-slate-600 dark:text-slate-400">{d.returnReason || d.reason || '—'}</span>
                  </td>
                  <td className="px-4 py-3"><span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{formatCurrency(d.totalAmount)}</span></td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => {
                        ...
                        className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-gray-700/60 text-slate-400 dark:text-slate-500 hover:text-emerald-600 transition-colors" title="Share"><MessageSquare className="w-3.5 h-3.5" /></button>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-slate-500 dark:text-slate-400">{filtered.length} debit notes</p>
