...
import {
  ArrowLeft, Truck, Phone, Mail, MapPin, Receipt, AlertCircle, TrendingUp,
...
const SupplierLedger = () => {
    ...
    const load = async () => {
      try {
        const [ledgerRes, noteRes] = await Promise.allSettled([
          ledgerAPI.getSupplier(id),
          ledgerNoteAPI.get('supplier', id),
        ]);
  ...
  const saveNote = async () => {
    try {
      await ledgerNoteAPI.save('supplier', id, note);
  ...
  const handlePrint = () => {
    ...
    const rows = filteredEntries.map((e, i) => {
      ...
      return `<tr${i % 2 === 0 ? '' : ' style="background:#f9fafb"'}>
        ...
        <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;text-align:right;color:#059669">${e.debit > 0 ? formatCurrency(e.debit) : '-'}</td>
        <td style="padding:8px 12px;border-bottom:1px solid #f3f4f6;text-align:right;color:#dc2626">${e.credit > 0 ? formatCurrency(e.credit) : '-'}</td>
    ...
    w.document.write(`
      <html><head><title>Supplier Ledger - ${data.supplier.name}</title>
      <style>
      ...
      <div class="header">
        <div class="info">
          ...
          <p>${data.supplier.phone || ''}${data.supplier.phone && data.supplier.email ? ' | ' : ''}${data.supplier.email || ''}</p>
        </div>
        <div class="balance">
          <div class="lbl">Current Balance</div>
          <div class="val" style="color:${data.outstandingPayable > 0 ? '#dc2626' : '#059669'}">${formatCurrency(data.outstandingPayable)}</div>
        </div>
      </div>
      <table>
        <thead><tr>
      ...
      <div class="summary">
        ...
        <div><div class="lbl">Total Purchases</div><div class="val">${formatCurrency(data.totalPurchases)}</div></div>
        <div><div class="lbl">Outstanding Payable</div><div class="val" style="color:${data.outstandingPayable > 0 ? '#dc2626' : '#059669'}">${formatCurrency(data.outstandingPayable)}</div></div>
      </div>
  ...
  const handleDownloadPDF = async () => {
    try {
      const printContent = `
        <html><head><title>Ledger - ${data.supplier.name}</title>
        ...
        <h2>Supplier Ledger: ${data.supplier.name}</h2>
        <p>Phone: ${data.supplier.phone || '-'} | Email: ${data.supplier.email || '-'}</p>
        <p>Outstanding Payable: ${formatCurrency(data.outstandingPayable)}</p>
  ...
  const handleExportExcel = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `ledger-${data.supplier.name}.csv`; a.click();
  ...
  const handleShare = async () => {
    try {
      if (navigator.share) {
        await navigator.share({ title: `Ledger - ${data.supplier.name}`, text: `Supplier Ledger: ${data.supplier.name}\nBalance: ${formatCurrency(data.outstandingPayable)}` });
      } else {
  ...
  const entryTypeIcon = (type) => {
    ...
    if (type === 'Purchase') return <ShoppingCart className="w-3.5 h-3.5" />;
  ...
  const entryTypeColor = (type) => {
    ...
    if (type === 'Purchase') return 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300';
  ...
  const balanceColor = data.outstandingPayable === 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400';
  const dueStatusBadge = data.outstandingPayable === 0
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <button onClick={() => navigate('/suppliers')} className="inline-flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors w-fit">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 dark:border-gray-700/80 shadow-soft p-5">
          <div className="flex items-center gap-4 mb-3">
            <div className="p-2.5 bg-amber-50 dark:bg-amber-900/30 rounded-xl"><Truck className="w-5 h-5 text-amber-600 dark:text-amber-400" /></div>
            <div>
              <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">{data.supplier.name}</h1>
          ...
          <div className="flex flex-wrap gap-4 text-sm text-slate-500 dark:text-slate-400">
            {data.supplier.phone && <span className="flex items-center gap-1.5"><Phone className="w-3.5 h-3.5" /> {data.supplier.phone}</span>}
            {data.supplier.email && <span className="flex items-center gap-1.5"><Mail className="w-3.5 h-3.5" /> {data.supplier.email}</span>}
            {data.supplier.address && <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> {data.supplier.address}</span>}
          </div>
        </div>
        <div className="sm:w-56 bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 dark:border-gray-700/80 shadow-soft p-5 flex flex-col items-center justify-center">
          ...
          <p className={`text-2xl font-bold mt-1 ${balanceColor}`}>{formatCurrency(data.outstandingPayable)}</p>
          {data.outstandingPayable > 0 && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 dark:border-gray-700/80 shadow-soft p-4">
          <div className="flex items-center gap-2">
            ...
            <div><p className="text-xs text-slate-500 dark:text-slate-400">Total Purchases</p><p className="text-lg font-bold text-slate-900 dark:text-slate-100">{formatCurrency(data.totalPurchases)}</p></div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 dark:border-gray-700/80 shadow-soft p-4">
          <div className="flex items-center gap-2">
            ...
            <div><p className="text-xs text-slate-500 dark:text-slate-400">Outstanding Payable</p><p className="text-lg font-bold text-red-600 dark:text-red-400">{formatCurrency(data.outstandingPayable)}</p></div>
          </div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-gray-700/50">
                {filteredEntries.map((e, i) => (
                  <tr key={i} className={`${e.type === 'Opening Balance' ? 'bg-gray-50/50 dark:bg-gray-700/30 font-medium' : 'hover:bg-gray-50/30 dark:hover:bg-gray-700/30'} transition-colors`}>
                    ...
                    <td className={`px-5 py-3 text-right font-medium ${e.debit > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400'}`}>{e.debit > 0 ? formatCurrency(e.debit) : '-'}</td>
                    <td className={`px-5 py-3 text-right font-medium ${e.credit > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400'}`}>{e.credit > 0 ? formatCurrency(e.credit) : '-'}</td>
              ...
              <div key={i} className="flex gap-4 pb-4 relative">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <p className={`text-sm font-medium ${e.type === 'Purchase' ? 'text-amber-700 dark:text-amber-300' : 'text-slate-900 dark:text-slate-100'}`}>
                      {e.type}
                    </p>
      ...
      {activeTab === 'analytics' && data.analytics && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 dark:border-gray-700/80 shadow-soft p-5">
            <div className="flex items-center gap-3 mb-3">
              <div className="p-2 bg-amber-50 dark:bg-amber-900/30 rounded-lg"><ShoppingCart className="w-4 h-4 text-amber-600 dark:text-amber-400" /></div>
              <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Purchases</p>
            </div>
            <p className="text-lg font-bold text-slate-900 dark:text-slate-100">{formatNumber(data.analytics.purchaseCount)}</p>
          </div>
        </div>
      )}
