      ...
      <motion.div variants={itemVariants} className="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200/80 shadow-soft p-4">
        <div className="relative">
          ...
          <input type="text" value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setCurrentPage(1); }}
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-gray-700 border border-slate-200 dark:border-gray-700 rounded-xl text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
          />
        </div>
      </motion.div>
      ...
      {payments.length === 0 ? (
        ...
        >
          <div className="flex flex-col items-center text-center max-w-sm mx-auto">
            ...
            <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 mb-2">No Transactions to Show</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Record your first outgoing payment to a supplier or vendor.</p>
            <button onClick={openAddModal}
        ...
        >
              ...
              <tbody>
                {paginated.map((p, idx) => (
                  <tr key={p._id} className="group border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{formatDate(p.date)}</td>
                    <td className="px-4 py-3"><span className="text-sm font-semibold text-slate-900 dark:text-slate-100 font-mono">{p.reference || '-'}</span></td>
                    <td className="px-4 py-3"><span className="text-sm font-medium text-slate-900 dark:text-slate-100">{p.partyName}</span></td>
                    <td className="px-4 py-3">
                      <span className="text-xs font-medium text-slate-600 dark:text-slate-400 capitalize px-2 py-0.5 bg-slate-100 dark:bg-gray-700 rounded">{p.paymentMethod}</span>
                    </td>
                    <td className="px-4 py-3"><span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{formatCurrency(p.amount)}</span></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEditModal(p)}
                          className="p-1.5 rounded-lg hover:bg-blue-50 text-slate-400 dark:text-slate-500 hover:text-blue-600 transition-colors" title="Edit"
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 dark:border-gray-700">
            <p className="text-xs text-slate-500 dark:text-slate-400">{filtered.length} payment{filtered.length !== 1 ? 's' : ''} · Total: <span className="font-semibold text-slate-700 dark:text-slate-300">{formatCurrency(totalAmount)}</span></p>
            <div className="flex items-center gap-1">
              ...
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(pg => (
                <button key={pg} onClick={() => setCurrentPage(pg)}
                  className={`w-7 h-7 rounded-lg text-xs font-medium transition-colors ${currentPage === pg ? 'bg-blue-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-gray-700/60'}`}
                >{pg}</button>
              ))}
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 pt-5 pb-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => { setShowModal(false); setEditingId(null); }} className="p-1.5 text-slate-500 dark:text-slate-400 hover:bg-red-50 hover:text-red-500 rounded-lg transition-colors" title="Close">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                          ...
                          <>
                            ...
                            >
                              ...
                              {PAYMENT_MODES.every(m => availableTypes.includes(m)) && (
                                <p className="px-2 py-1.5 text-xs text-slate-400 dark:text-slate-500">All payment types added</p>
                              )}
                            </motion.div>
                          </>
                        )}
                    ...
                    <div>
                      <button onClick={() => setShowDesc(true)}
                        className={`inline-flex items-center gap-2 px-3 py-2 text-xs font-medium rounded-lg transition-all ${form.description ? 'text-emerald-700 border border-emerald-200 bg-emerald-50' : 'text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-gray-700 hover:border-blue-300 hover:bg-blue-50'}`}
                      >
                    ...
                    <div>
                      ...
                      <button onClick={() => fileInputRef.current?.click()}
                        className="p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-gray-700/60 rounded-lg transition-colors"
                        title="Add Image"
                      >
                  ...
                  <AnimatePresence>
                    {shareOpen && (
                      <>
                        ...
                        <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                          className="absolute right-0 bottom-full mb-1 w-44 bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-lg shadow-lg z-20 p-1"
                        >
                          {[
                            ...
                            { label: 'Copy Link', icon: Link2, color: 'text-slate-600 dark:text-slate-400', action: () => { const { message } = generateShareContent('payment', form); copyToClipboard(message); } },
                          ].map(opt => (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowDesc(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowDesc(false)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600"><X className="w-5 h-5" /></button>
              </div>
              <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={6}
                placeholder="Add a description or note..." className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-gray-700 rounded focus:outline-none focus:border-blue-500 resize-none"
              />
              <div className="flex justify-end gap-2 mt-3">
                <button onClick={() => setForm(f => ({ ...f, description: '' }))} className="px-3 py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-gray-700/60 rounded">Clear</button>
            ...
            <div className="px-4 py-4 bg-slate-900">
              <div className="text-right">
                ...
                {operator && previous != null && (
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{previous} {operator}</p>
                )}
              </div>
            </div>
