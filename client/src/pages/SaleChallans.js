...
const SaleChallans = () => {
  ...
  const [filters, setFilters] = useState({
    deliveryStatus: '', customer: '', datePreset: '',
  ...
  const loadChallans = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await saleAPI.getAll({
        page, limit: 50, search, type: 'challan', ...filters,
  ...
  const handleMarkDelivered = async (id) => {
    try {
      await saleAPI.update(id, { deliveryStatus: 'delivered' });
  ...
  const handleDelete = async (id) => {
    ...
    try { await saleAPI.delete(id); toast.success('Challan cancelled'); loadChallans(); }
  ...
  const resetFilters = () => {
    setFilters({ deliveryStatus: '', customer: '', datePreset: '', dateFrom: '', dateTo: '', sortBy: 'date', sortOrder: 'desc' });
    setSearch(''); setPage(1);
  };

  const activeFilterCount = [filters.deliveryStatus, filters.customer, filters.datePreset].filter(Boolean).length;

  const deliveryBadge = (status) => {
    if (status === 'delivered') return <Badge variant="paid">Delivered</Badge>;
    if (status === 'cancelled') return <Badge variant="cancelled">Cancelled</Badge>;
    return <Badge variant="unpaid">Pending</Badge>;
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Delivery Challans</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{total} challan{total !== 1 ? 's' : ''}</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search challans..." className="w-48 lg:w-64 pl-9 pr-3 py-2 border border-slate-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" />
          </div>
          ...
          <Link to="/sales/new" state={{ documentType: 'challan' }} className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-slate-200/80 dark:border-gray-700/80 shadow-soft p-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-amber-50 dark:bg-amber-500/10 rounded-xl"><Truck className="w-5 h-5 text-amber-600 dark:text-amber-400" /></div>
            <div><p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Total Challans</p><p className="text-lg font-bold text-slate-900 dark:text-slate-100 mt-0.5">{total}</p></div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-slate-200/80 dark:border-gray-700/80 shadow-soft p-4">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Delivered</p><p className="text-lg font-bold text-slate-900 dark:text-slate-100 mt-0.5">{challans.filter(c => c.deliveryStatus === 'delivered').length}</p></div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-slate-200/80 dark:border-gray-700/80 shadow-soft p-4">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-red-50 dark:bg-red-500/10 rounded-xl"><Clock className="w-5 h-5 text-red-600 dark:text-red-400" /></div>
            <div><p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Pending</p><p className="text-lg font-bold text-slate-900 dark:text-slate-100 mt-0.5">{challans.filter(c => c.deliveryStatus === 'pending').length}</p></div>
          </div>
        </div>
      </div>
      ...
      {showFilters && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-gray-800 rounded-xl border border-slate-200 dark:border-gray-700 shadow-soft p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Delivery Status</label>
              <select value={filters.deliveryStatus} onChange={(e) => setFilters(f => ({ ...f, deliveryStatus: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-700 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
      ...
      {loading ? <LoadingSpinner /> : challans.length === 0 ? (
        <EmptyState icon={Truck} title="No Challans Yet" description="Delivery challans will appear here." actionLabel="New Challan" onAction={() => navigate('/sales/new', { state: { documentType: 'challan' } })} />
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-gray-700 bg-slate-50/80 dark:bg-gray-800/80">
                  ...
                  <th className="px-4 py-3 text-left text-2xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Challan No.</th>
                  ...
                  <th className="px-4 py-3 text-left text-2xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest hidden md:table-cell">Vehicle</th>
                  <th className="px-4 py-3 text-left text-2xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest hidden lg:table-cell">E-Way Bill</th>
                  <th className="px-4 py-3 text-center text-2xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Delivery</th>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-gray-700/50">
                {challans.map((c) => (
                  <tr key={c._id} className="hover:bg-slate-50/60 dark:hover:bg-gray-700/20 transition-colors">
                    <td className="px-4 py-3 text-slate-500 dark:text-slate-400 whitespace-nowrap">{formatDate(c.date)}</td>
                    <td className="px-4 py-3">
                      <Link to={`/sales/challans/${c._id}`} className="font-medium text-amber-600 dark:text-amber-400 hover:underline">{c.invoiceNumber}</Link>
                    </td>
                    <td className="px-4 py-3 text-slate-900 dark:text-slate-100 truncate max-w-[150px]">{c.customerName || 'Walk-in'}</td>
                    <td className="px-4 py-3 text-slate-500 dark:text-slate-400 hidden md:table-cell">{c.vehicleNo || '-'}</td>
                    <td className="px-4 py-3 text-slate-500 dark:text-slate-400 hidden lg:table-cell">{c.eWayBill || '-'}</td>
                    <td className="px-4 py-3 text-center">{deliveryBadge(c.deliveryStatus)}</td>
                    <td className="px-4 py-3 text-center relative">
                      <div className="flex items-center justify-center gap-0.5">
                        <Link to={`/sales/challans/${c._id}`} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-gray-700 text-slate-500 dark:text-slate-400 transition-colors" title="View"><Eye className="w-3.5 h-3.5" /></Link>
                        <button onClick={() => handleDownloadPDF(c)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-gray-700 text-slate-500 dark:text-slate-400 transition-colors" title="Download PDF"><Download className="w-3.5 h-3.5" /></button>
                        {c.deliveryStatus === 'pending' && (
                          <button onClick={() => handleMarkDelivered(c._id)} className="p-1.5 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 transition-colors" title="Mark Delivered"><CheckCircle className="w-3.5 h-3.5" /></button>
                        )}
                        <button onClick={() => handleDelete(c._id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-red-500 dark:text-red-400 transition-colors" title="Cancel"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
