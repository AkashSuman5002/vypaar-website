      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
        ...
        <motion.div variants={itemVariants} className="bg-white rounded-2xl border border-slate-200/80 shadow-soft p-12 lg:p-16">
          <div className="flex flex-col items-center text-center max-w-lg mx-auto">
            ...
            <p className="text-sm text-slate-500 mb-8">Create purchase orders, approve them, and track partial receives.</p>
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Purchase Order</h1>
          <p className="text-sm text-slate-500 mt-0.5">Create, approve, receive, and track purchase orders</p>
        </div>
        <button onClick={() => { resetForm(); setShowModal(true); }}
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-soft p-4 lg:p-5">
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-xl ${partiallyReceivedCount > 0 ? 'bg-purple-50' : 'bg-slate-50'}`}>
              <Package className={`w-5 h-5 ${partiallyReceivedCount > 0 ? 'text-purple-600' : 'text-slate-400'}`} />
            </div>
            <div><p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Partial</p><p className={`text-xl font-bold mt-1 ${partiallyReceivedCount > 0 ? 'text-purple-600' : 'text-slate-900'}`}>{partiallyReceivedCount}</p></div>
          </div>
        </div>
      </motion.div>
                ...
                return (
                  <tr key={o._id} className="group border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-slate-600">
                      ...
                      ) : <span className="text-slate-400">0/{totalOrdered}</span>}
                    </td>
                      ...
                      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        {['draft', 'pending'].includes(o.status) && (
                          <button onClick={() => handleApprove(o)}
                            className="p-1.5 rounded-lg hover:bg-emerald-50 text-slate-400 hover:text-emerald-600 transition-colors" title="Approve">
                            <CheckCircle className="w-3.5 h-3.5" />
                          </button>
                        )}
                        {['ordered', 'partially_received'].includes(o.status) && (
                          <button onClick={() => openReceiveModal(o)}
                            className="p-1.5 rounded-lg hover:bg-blue-50 text-slate-400 hover:text-blue-600 transition-colors" title="Receive Stock">
                        ...
                        <button onClick={() => handleDuplicate(o)}
                          className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-purple-600 transition-colors" title="Duplicate">
        ...
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              className="relative bg-white rounded-2xl shadow-elevated w-full max-w-4xl max-h-[92vh] overflow-hidden border border-slate-200/80">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                <h3 className="text-base font-semibold text-slate-900">{editing ? 'Edit' : 'New'} Purchase Order</h3>
                      ...
                      <tbody>
                        {items.map((item, idx) => (
                          <tr key={idx} className="border-t border-slate-100">
                            ...
                            <td className="px-3 py-2">
                              {items.length > 1 && (
                                <button onClick={() => removeItem(idx)} className="p-1 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-500">
        ...
        {showReceiveModal && selectedOrder && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                <div>
                  <h3 className="text-base font-semibold text-slate-900">Receive Stock</h3>
                  <p className="text-xs text-slate-500 mt-0.5">{selectedOrder.orderNumber} — {selectedOrder.supplierName}</p>
                </div>
                <button onClick={() => setShowReceiveModal(false)} className="p-1.5 rounded-lg hover:bg-slate-100"><X className="w-4.5 h-4.5 text-slate-500" /></button>
              </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200">
                        ...
                        <th className="px-4 py-2.5 text-center text-2xs font-semibold text-slate-500 uppercase">Ordered</th>
                        <th className="px-4 py-2.5 text-center text-2xs font-semibold text-slate-500 uppercase">Already Received</th>
                        <th className="px-4 py-2.5 text-center text-2xs font-semibold text-slate-500 uppercase">Pending</th>
                        <th className="px-4 py-2.5 text-center text-2xs font-semibold text-slate-500 uppercase w-28">Receive Now</th>
                      </tr>
                    </thead>
                    <tbody>
                      {receiveForm.items.map((item, idx) => (
                        <tr key={idx} className="border-b border-slate-100 last:border-0">
                          ...
                          <td className="px-4 py-2.5 text-sm text-center text-slate-500">{item.orderedQuantity}</td>
              ...
              <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                <button onClick={() => setShowReceiveModal(false)} className="px-4 py-2.5 text-sm font-medium text-slate-600 rounded-xl hover:bg-slate-100">Cancel</button>
        ...
        {showCancelModal && selectedOrder && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.96, y: 8 }}
              transition={{ type: 'spring', duration: 0.3, bounce: 0.2 }}
              className="relative bg-white rounded-2xl shadow-elevated w-full max-w-md overflow-hidden border border-slate-200/80">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                <h3 className="text-base font-semibold text-slate-900">Cancel Order</h3>
                <button onClick={() => setShowCancelModal(false)} className="p-1.5 rounded-lg hover:bg-slate-100"><X className="w-4.5 h-4.5 text-slate-500" /></button>
              </div>
              <div className="p-6 space-y-4">
                <p className="text-sm text-slate-600">Are you sure you want to cancel order <strong>{selectedOrder.orderNumber}</strong>?</p>
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">Reason (Optional)</label>
              ...
              <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                <button onClick={() => setShowCancelModal(false)} className="px-4 py-2.5 text-sm font-medium text-slate-600 rounded-xl hover:bg-slate-100">Keep Order</button>
